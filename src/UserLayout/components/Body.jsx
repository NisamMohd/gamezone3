...
function Body() {
  ...
  // Track scroll progress across the 450vh tall container for the 3D controller section
  ...
  // ============================================================
  // DYNAMIC VIDEO MOTION TRANSFORMS (CONTROLLER VIDEO MOVES ALONG WITH SCROLL)
  // ============================================================
  ...
  // ============================================================
  // TEXT STORYTELLING TRANSFORMS ACROSS CONTROLLER SCROLL
  // ============================================================
  ...
  // Update targetProgress whenever scrollYProgress changes
  useMotionValueEvent(scrollYProgress, 'change', (latest) => {
    targetProgress.current = latest
    setScrollPercent(Math.round(latest * 100))
  })

  // Ultra-robust lerp animation loop for controller video scrubbing
  useEffect(() => {
    ...
    // Sync initial scroll value immediately
    targetProgress.current = scrollYProgress.get() || 0
    currentProgress.current = targetProgress.current

    const updateVideoFrame = () => {
      if (!durationRef.current && video.duration && !isNaN(video.duration)) {
        durationRef.current = video.duration
        setIsVideoReady(true)
      }
      ...
      if (durationRef.current > 0 && !video.seeking) {
        const targetTime = Math.min(
          durationRef.current - 0.02,
          Math.max(0, currentProgress.current * durationRef.current)
        )
  ...
  return (
    <div ref={containerRef} className="relative h-[450vh] w-full bg-[#05070C] border-t border-cyan-500/10">
      ...
      <section className="sticky top-0 h-screen w-full overflow-hidden bg-[#05070C]">
        ...
        >
          ...
          >
            ...
            <source src="/video/controller.mp4" type="video/mp4" />
          </video>
        </motion.div>
        ...
        <div className="relative z-20 flex h-full w-full flex-col items-end justify-center px-8 md:px-16 lg:px-24">
          {/* PHASE 1: HERO CONTROLLER INTRODUCTION (0% - 30%) */}
          <motion.div
            style={{
          ...
          >
            <div className="font-mono text-xs tracking-[0.3em] text-[#00E5FF] mb-4 flex items-center gap-2">
              ...
              SYSTEM ONLINE — 360° CONTROLLER TOUR
            </div>

            <h2 className="font-rajdhani font-bold text-5xl md:text-7xl leading-[0.95] text-white uppercase tracking-tight">
              Gear up for
          ...
          >
            ...
            <h3 className="font-rajdhani font-bold text-4xl md:text-5xl leading-[0.95] text-white uppercase tracking-tight">
              Deconstructed
          ...
          >
            ...
            <h3 className="font-rajdhani font-bold text-4xl md:text-6xl leading-[0.95] text-white uppercase tracking-tight">
              Dominate the
